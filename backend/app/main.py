...
DEFAULT_OPD_PATIENTS = [
    {"patient_id": 1, "token": "A-001", "name": "Rahul Sharma", "department": "General Medicine", "status": "Completed"},
    {"patient_id": 2, "token": "A-002", "name": "Priya Reddy", "department": "General Medicine", "status": "Completed"},
    {"patient_id": 3, "token": "A-003", "name": "Arjun Kumar", "department": "General Medicine", "status": "Completed"},
    {"patient_id": 4, "token": "A-004", "name": "Sneha Rao", "department": "General Medicine", "status": "Completed"},
    {"patient_id": 5, "token": "A-005", "name": "Vikram Singh", "department": "General Medicine", "status": "Completed"},
    {"patient_id": 6, "token": "A-006", "name": "Ananya Das", "department": "General Medicine", "status": "Consulting"},
    {"patient_id": 7, "token": "A-007", "name": "Kiran Patel", "department": "General Medicine", "status": "Waiting"},
    {"patient_id": 8, "token": "A-008", "name": "Meena Joseph", "department": "General Medicine", "status": "Waiting"},
    {"patient_id": 9, "token": "A-009", "name": "Rohit Verma", "department": "General Medicine", "status": "Waiting"},
    {"patient_id": 10, "token": "A-010", "name": "Rahul Sharma", "department": "General Medicine", "status": "Waiting"}
]
...
def login(request: LoginRequest, db: Session = Depends(get_db)):
    ...
    if role == "patient":
        ...
        if request.healthflow_id:
            patient = db.query(Patient).filter(Patient.healthflow_id == request.healthflow_id).first()
        if not patient and request.patient_id:
            patient = db.query(Patient).filter(Patient.id == request.patient_id).first()
        if not patient:
            patient = db.query(Patient).filter(Patient.id == 10).first()
        ...
        return {
            ...
            "user": {
                ...
                "healthflow_id": patient.healthflow_id if patient else "HF-2026-00142",
                ...
                "allergies": patient.allergies if patient else "Penicillin",
                "phone": patient.phone if patient else "+91 98765 12345",
                "emergency_contact": patient.emergency_contact if patient else "Priya Sharma",
                "emergency_phone": patient.emergency_phone if patient else "+91 98765 43210"
            }
        }
        ...
        return {
            ...
            "user": {
                ...
                "name": doctor.name if doctor else "Dr. Anil Kumar",
                "department": doctor.department if doctor else "General Medicine",
                "specialization": doctor.specialization if doctor else "Consultant Physician",
                "email": doctor.email if doctor else "anil.kumar@healthflow.org"
            }
        }
...
def get_patient_by_healthflow_id(healthflow_id: str, db: Session = Depends(get_db)):
    patient = db.query(Patient).filter(Patient.healthflow_id == healthflow_id).first()
    if not patient:
        raise HTTPException(status_code=404, detail="Patient not found")
    return patient

@app.get("/patients/token/{token}")
def get_patient_by_token(token: str, db: Session = Depends(get_db)):
    t = token.upper()
    p_match = next((p for p in opd_patients if p["token"] == t), None)
    if p_match:
...
def create_patient(data: PatientCreate, db: Session = Depends(get_db)):
    existing = db.query(Patient).filter(Patient.healthflow_id == data.healthflow_id).first()
    if existing:
        raise HTTPException(status_code=400, detail="HealthFlow ID already exists")
...
def update_doctor_availability(doctor_id: int, available: bool, db: Session = Depends(get_db)):
...
def get_opd_queue(db: Session = Depends(get_db)):
    waiting = [p for p in opd_patients if p["status"] == "Waiting"]
    consulting = [p for p in opd_patients if p["status"] == "Consulting"]
    completed = [p for p in opd_patients if p["status"] == "Completed"]

    active_docs_count = db.query(Doctor).filter(Doctor.is_available == 1).count()
...
def get_patient_prediction(token: str, db: Session = Depends(get_db)):
    token = token.upper()
    patient_index = next((i for i, p in enumerate(opd_patients) if p["token"] == token), None)

    active_docs = db.query(Doctor).filter(Doctor.is_available == 1).count()
    if active_docs <= 0:
    ...
    if patient_index is None:
        return {
            ...
            "estimated_waiting_time": round((3 * AVERAGE_CONSULTATION_DURATION) / max(active_docs, 1)),
    ...
    if patient["status"] == "Consulting":
        return {
            ...
            "doctors_available": active_docs,
            "average_consultation_duration": AVERAGE_CONSULTATION_DURATION,
            "patients_currently_being_served": len(consulting),
            "estimated_waiting_time": 0,
            "confidence_level": "High"
        }

    waiting_before = [p for p in opd_patients[:patient_index] if p["status"] == "Waiting"]
    patients_ahead = len(waiting_before)

    estimated_wait = round((patients_ahead * AVERAGE_CONSULTATION_DURATION) / max(active_docs, 1))
...
@app.post("/opd/join")
def join_opd_queue(request: JoinQueueRequest):
    existing = next((p for p in opd_patients if p["patient_id"] == request.patient_id), None)
    if existing:
        return {"message": "Already in queue", "patient": existing}
    ...
    new_item = {
        ...
        "token": new_token,
        "name": request.name,
        "department": request.department or "General Medicine",
        "status": "Waiting"
    }
    opd_patients.append(new_item)
    return {"message": "Joined OPD queue successfully", "patient": new_item}
...
def call_next_patient(db: Session = Depends(get_db)):
    ...
    consulting_count = sum(1 for p in opd_patients if p["status"] == "Consulting")
    if consulting_count >= active_docs:
        return {
            "message": "All doctors are currently consulting. Please complete an active consultation first.",
            "patient": None
        }

    waiting_patient = next((p for p in opd_patients if p["status"] == "Waiting"), None)
    ...
    return {
        "message": f"Called patient {waiting_patient['name']} (Token {waiting_patient['token']})",
        "patient": waiting_patient
    }

@app.post("/opd/complete/{token}")
def complete_opd_patient(token: str):
    token = token.upper()
    patient = next((p for p in opd_patients if p["token"] == token), None)
    if not patient:
        raise HTTPException(status_code=404, detail="Patient token not found")

    patient["status"] = "Completed"
    return {
        "message": f"Consultation for token {token} marked as completed",
        "patient": patient
    }
...
def reset_opd_queue():
    ...
    opd_patients = [p.copy() for p in DEFAULT_OPD_PATIENTS]
    return {
        "message": "OPD queue reset to demonstration initial state",
        "patients": opd_patients
    }
...
def analyze_lab_report(request: LabAnalysisRequest, db: Session = Depends(get_db)):
    test_key = request.test_name.strip().lower()
    value = request.value
    gender = (request.gender or "unknown").strip().lower()

    if test_key not in LAB_TESTS:
        raise HTTPException(status_code=400, detail=f"Unsupported test '{request.test_name}'")
    ...
    if test_key == "hemoglobin":
        if gender == "male":
            ...
            low = min(test_info["male_range"][0], test_info["female_range"][0])
            high = max(test_info["male_range"][1], test_info["female_range"][1])
    else:
    ...
    ref_range_str = f"{low} - {high} {test_info['unit']}"
    doc_discussion = "Discuss this result with your doctor. Your doctor can interpret it together with your clinical symptoms and health history."
    disclaimer = "HealthFlow provides educational explanations. It does not provide medical diagnoses or prescribe treatment."
    ...
    audit = AuditLog(
        ...
        details=f"Analyzed {request.test_name} result ({value} {test_info['unit']}) with status {status}"
    )
    db.add(audit)
    db.commit()
...
def get_patient_lab_reports(patient_id: int, db: Session = Depends(get_db)):
    reports = db.query(LabReport).filter(LabReport.patient_id == patient_id).order_by(LabReport.created_at.desc()).all()
    formatted = []
    for r in reports:
        formatted.append({
            "id": r.id,
            ...
            "condition_context": r.health_context or "General",
            "explanation": r.plain_language_explanation,
            "doctor_discussion": r.doctor_discussion,
...
@app.get("/lab/all")
def get_all_lab_reports(db: Session = Depends(get_db)):
    reports = db.query(LabReport).order_by(LabReport.created_at.desc()).all()
    return {
...
def get_supported_lab_tests():
    ...
    for name, item in LAB_TESTS.items():
        if name == "hemoglobin":
            ref = f"{item['female_range'][0]}-{item['female_range'][1]} (F) / {item['male_range'][0]}-{item['male_range'][1]} (M) {item['unit']}"
        else:
            ref = f"{item['range'][0]} - {item['range'][1]} {item['unit']}"
        tests.append({
            "test_name": name,
            "title": name.title(),
            "unit": item["unit"],
...
@app.post("/records")
def create_medical_record(data: RecordCreate, db: Session = Depends(get_db)):
    new_rec = MedicalRecord(**data.model_dump())
    db.add(new_rec)
    ...
    audit = AuditLog(
        ...
        details=f"Added consultation notes: {data.diagnosis or 'Consultation note'}"
    )
    db.add(audit)
    db.commit()
    db.refresh(new_rec)
...
def check_patient_consent(patient_id: int, requester: Optional[str] = None, db: Session = Depends(get_db)):
    query = db.query(Consent).filter(Consent.patient_id == patient_id, Consent.status == "granted")
    if requester:
        query = query.filter(Consent.requester_name.ilike(f"%{requester}%"))
    active = query.first()
    return {
        "patient_id": patient_id,
...
@app.post("/consent")
def create_consent_request(data: ConsentCreate, db: Session = Depends(get_db)):
    expires = datetime.now() + timedelta(hours=data.duration_hours or 24)
    new_consent = Consent(
        patient_id=data.patient_id,
    ...
    audit = AuditLog(
        ...
        details=f"Access requested for {data.access_type}"
    )
    db.add(audit)
    db.commit()
...
def update_consent_decision(consent_id: int, decision: str, db: Session = Depends(get_db)):
    decision_clean = decision.lower().strip()
    if decision_clean not in ["granted", "rejected", "revoked"]:
        raise HTTPException(status_code=400, detail="Invalid decision. Must be granted, rejected, or revoked.")
    ...
    if decision_clean == "granted":
        consent.expires_at = datetime.now() + timedelta(hours=24)

    audit = AuditLog(
        patient_id=consent.patient_id,
        actor_name="Patient",
        actor_role="Patient",
        action=f"Consent {decision_clean.title()}",
        resource_type="Consent",
        resource_id=consent.id,
        details=f"Consent decision changed to '{decision_clean}' for {consent.requester_name}"
    )
    db.add(audit)
    db.commit()
    db.refresh(consent)
...
def get_patient_audit_logs(patient_id: int, db: Session = Depends(get_db)):
    logs = db.query(AuditLog).filter(AuditLog.patient_id == patient_id).order_by(AuditLog.created_at.desc()).all()
    return logs

@app.post("/audit")
def create_audit_log(data: AuditCreate, db: Session = Depends(get_db)):
...
def get_patient_appointments(patient_id: int, db: Session = Depends(get_db)):
    apps = db.query(Appointment).filter(Appointment.patient_id == patient_id).order_by(Appointment.appointment_time.asc()).all()
    return apps

@app.post("/appointments")
def create_appointment(data: AppointmentCreate, db: Session = Depends(get_db)):
    app_time = data.appointment_time or (datetime.now() + timedelta(days=1, hours=3))
    new_app = Appointment(
        patient_id=data.patient_id,
...
def get_admin_summary(db: Session = Depends(get_db)):
    waiting = sum(1 for p in opd_patients if p["status"] == "Waiting")
    consulting = sum(1 for p in opd_patients if p["status"] == "Consulting")
    completed = sum(1 for p in opd_patients if p["status"] == "Completed")
    ...
    completion_rate = round((completed / total_opd) * 100) if total_opd else 0
